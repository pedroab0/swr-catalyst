...
2.  **Update Linux snapshots (via CI):** 
    - Push your changes to a branch.
    - The GitHub Action will fail because the Linux snapshots don't match.
    - Go to the **Actions** tab in GitHub and click on the failed run.
    - Scroll down to **Artifacts** and download the `playwright-report`.
    - Inside that zip, you will find the "Actual" images generated by the CI.
    - Copy those images into the snapshots directory (ensuring they end in `-linux.png`).
    - Commit and push the new Linux baselines.

This ensures that the CI environment remains the source of truth for Linux rendering, avoiding discrepancies caused by local hardware differences (e.g., ARM64 vs AMD64).
