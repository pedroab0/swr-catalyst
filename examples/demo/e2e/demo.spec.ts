...
test.describe('swr-catalyst demo app', () => {
  ...
  test('should perform basic CRUD operations', async ({ page }) => {
    ...
    const todoItem = page.locator('.todoList li').filter({ hasText: 'E2E Test Todo' });
    ...
    const renamedTodoItem = page.locator('.todoList li').filter({ hasText: 'Renamed E2E Todo' });
  ...
  test('should show validation error in error inspector', async ({ page }) => {
    ...
    await expect(page.getByText(/Title must have at least 3 characters/).first()).toBeVisible();
    
    // Check visual state of error inspector
    await expect(page.locator('.panel').filter({ hasText: 'MutationError Inspector' })).toHaveScreenshot('validation-error.png', {
      mask: [page.locator('.panel').filter({ hasText: 'MutationError Inspector' }).locator('pre')],
    });
  });

  test('should verify useStableKey referential stability', async ({ page }) => {
    const section = page.locator('.panel').filter({ hasText: 'useStableKey Demo' });
    const stabilityStatus = section.locator('div', { has: page.locator('dt', { hasText: 'Reference stable on last render?' }) }).locator('dd');
    
    // Initial state
    await expect(stabilityStatus).toHaveText('yes');

    // Re-render (should stay stable)
    await section.getByRole('button', { name: /Re-render only/ }).click();
    await expect(stabilityStatus).toHaveText('yes');

    // Toggle filter (should change reference)
    await section.getByRole('button', { name: /Toggle filter/ }).click();
  ...
  test('should replay actions from the timeline', async ({ page }) => {
    ...
    const timelineEntry = page.locator('.timelineItem').filter({ hasText: 'Create todo' }).first();
    ...
    await expect(page.locator('.todoList li').filter({ hasText: 'Replay Target' })).toHaveCount(2);
    
    // 4. Verify a "Replay timeline event" system event was added
    await expect(page.locator('.timelineItem').filter({ hasText: 'Replay timeline event' })).toBeVisible();
  });

  test('should show correct toast notifications', async ({ page }) => {
    // 1. Success toast
    await page.getByRole('button', { name: 'Reset demo state' }).click();
    await expect(page.locator('.toast-success')).toContainText('Demo state reset');
  ...
  test('should verify mutateByGroup targeting', async ({ page }) => {
    const utilities = page.locator('.panel').filter({ hasText: 'Utilities Demo' });
    
    // Target 'tasks' group
    await utilities.getByLabel('Target groups').fill('tasks');
    await utilities.getByRole('button', { name: 'mutateByGroup revalidate' }).click();

    // Verify status message
    await expect(utilities.getByText(/mutateByGroup revalidated/)).toBeVisible();
    
    // Verify timeline entry
    await expect(page.locator('.timelineItem').filter({ hasText: 'mutateByGroup revalidate' })).toBeVisible();
  });

  test('should verify low-level cache utilities and reset', async ({ page }) => {
    const utilities = page.locator('.panel').filter({ hasText: 'Utilities Demo' });
    ...
    await utilities.getByRole('button', { name: 'extractSWRKey parse' }).click();
    await expect(page.locator('.timelineItem').filter({ hasText: 'extractSWRKey parse' })).toBeVisible();

    // 2. resetCache
    await utilities.getByRole('button', { name: 'resetCache preserve app-config' }).click();
    await expect(page.locator('.toast-success')).toContainText('resetCache cleared');
  });
});
